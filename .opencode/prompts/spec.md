...
5. **Commit the session file**: If there were any recordings (user feedback or self-review):
   ...
   2. Run `git diff .opencode/sessions/spec/` to review the session changes
   3. Stage the session file: `git add .opencode/sessions/spec/<session-id>.yaml`
   4. Commit with message: `git commit -m "spec: record session feedback for <brief summary>"`
